...
const clientNav = [
  ...
  { label: 'Nouvelle mission', href: '/client/create', icon: Plus },
  { label: 'Mes devis', href: '/client/quotes', icon: FileText },
...
const artisanNav = [
  ...
  { label: 'Messagerie', href: '/artisan/messages', icon: MessageSquare },
];
  ...
  return (
    <ProtectedRoute requiredRole={['CLIENT', 'ARTISAN']}>
      <div className="min-h-screen bg-surface">
        ...
        {/* Sidebar */}
        <aside className={cn(
          'fixed top-0 left-0 z-50 h-full bg-white border-r border-border/50 transition-all duration-300 lg:translate-x-0',
          ...
          <div className="flex h-full flex-col">
            ...
            <div className="flex h-16 items-center justify-between px-4 border-b border-border/50">
              <Link href="/" className="flex items-center gap-2">
                ...
                <span className={cn('text-sm font-semibold text-foreground transition-all duration-300', !sidebarOpen && 'lg:opacity-0 lg:w-0 lg:overflow-hidden')}>Artisan237</span>
              </Link>
            ...
            <nav className="flex-1 overflow-y-auto p-3 space-y-1">
              {nav.map((item) => {
                ...
                const isActive = pathname === item.href || (item.href !== '/client' && item.href !== '/artisan' && pathname?.startsWith(item.href));
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                  ...
                  >
                    ...
                    <span className={cn('transition-all duration-300', !sidebarOpen && 'lg:opacity-0 lg:w-0 lg:overflow-hidden')}>{item.label}</span>
                    {isActive && (
                      <motion.div
            ...
            <div className="border-t border-border/50 p-3 space-y-2">
              ...
              >
                ...
                <span className={cn('transition-all duration-300', !sidebarOpen && 'lg:opacity-0 lg:w-0 lg:overflow-hidden')}>Déconnexion</span>
              </button>
            </div>
          </div>
        </aside>
        ...
        {/* Main content */}
        <div className={cn('transition-all duration-300', sidebarOpen ? 'lg:pl-64' : 'lg:pl-[72px]')}>
          {/* Top bar */}
