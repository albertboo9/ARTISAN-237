...
import { MessageSquare, Search, Send, Phone, Video, MoreHorizontal } from 'lucide-react';
import { cn } from '../../../lib/cn';
import { PageTransition } from '../../../components/shared/page-transition';

const conversations = [
  { id: '1', name: 'Paul Tchuente', lastMsg: 'Bonjour, je suis disponible pour intervenir', time: '14:30', unread: 2, online: true },
  { id: '2', name: 'Marc Ndjonga', lastMsg: 'D\'accord, je passe demain matin', time: 'Hier', unread: 0, online: false },
];
          ...
          {active && (
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {[
                  { text: 'Bonjour, je suis intéressé par votre service de plomberie.', sent: false, time: '14:25' },
                  { text: 'Bonjour ! Oui, je suis disponible. De quoi avez-vous besoin ?', sent: true, time: '14:28' },
                  { text: 'J\'ai une fuite d\'eau dans la salle de bain. Pouvez-vous passer cette semaine ?', sent: false, time: '14:30' },
                ].map((msg, i) => (
                  <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={cn('flex', msg.sent ? 'justify-end' : 'justify-start')}>
                    <div className={cn('max-w-[70%] px-4 py-2.5 rounded-2xl text-sm', msg.sent ? 'bg-primary text-primary-foreground rounded-br-md' : 'bg-surface-container text-foreground rounded-bl-md')}>
                      <p>{msg.text}</p>
                      <p className={cn('text-[10px] mt-1', msg.sent ? 'text-primary-foreground/60' : 'text-muted-foreground')}>{msg.time}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="p-4 border-t border-border/50">
                <div className="flex items-center gap-2">
                  <input value={input} onChange={e => setInput(e.target.value)} placeholder="Écrivez un message..." className="flex-1 h-10 px-4 text-sm rounded-xl border border-border bg-surface-container-low focus:outline-none focus:ring-2 focus:ring-primary/20" onKeyDown={e => e.key === 'Enter' && setInput('')} />
