...
export default function CreateMissionPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <p className="text-muted-foreground mt-1">Choisissez un métier, décrivez votre besoin en quelques étapes</p>

      <div className="flex items-center gap-2 mt-8 mb-10">
        {steps.map((s, i) => (
          <div key={s} className="flex items-center gap-2 flex-1">
            <div className={cn('flex h-8 w-8 items-center justify-center rounded-full text-xs font-semibold transition-all flex-shrink-0', i <= step ? 'bg-primary text-white' : 'bg-muted text-muted-foreground')}>
              {i < step ? <Check className="h-4 w-4" /> : i + 1}
            </div>
            <span className={cn('text-xs font-medium hidden sm:block truncate', i <= step ? 'text-foreground' : 'text-muted-foreground')}>{s}</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {services.length > 0 ? services.map((s) => (
                  <button key={s.id} onClick={() => { setSelectedService(s.id); setStep(1); }}
                    className={cn('p-4 rounded-xl border-2 text-left transition-all', selectedService === s.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30')}>
                    <h3 className="font-medium text-foreground">{s.name}</h3>
                    <p className="text-xs text-muted-foreground mt-1">{s.category?.name || 'Service général'}</p>
                  </button>
                )) : (
                  ['Plomberie', 'Électricité', 'Menuiserie', 'Peinture', 'Ménage', 'Climatisation'].map((s) => (
                    <button key={s} onClick={() => { setSelectedService(s); setStep(1); }}
                      className={cn('p-4 rounded-xl border-2 text-left transition-all', selectedService === s ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30')}>
        ...
        {step === 1 && (
          <div className="space-y-6">
            <div className="bento-card p-6 space-y-4">
              <Input label="Adresse détaillée" placeholder="Ex: Akwa, Rue Drouot, Douala" icon={<MapPin className="h-4 w-4" />} value={address} onChange={(e) => setAddress(e.target.value)} />
              <div className="bg-surface-container rounded-xl p-3 flex items-center justify-between">
                ...
                <button onClick={() => { if ('geolocation' in navigator) navigator.geolocation.getCurrentPosition((pos) => setPosition({ lat: pos.coords.latitude, lng: pos.coords.longitude })); }} className="text-xs text-primary font-medium hover:underline">Actualiser</button>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bento-card p-6 space-y-4">
              ...
              <div className="space-y-3 pt-2 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Service</span><span className="font-medium text-right">{getServiceName(selectedService)}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Adresse</span><span className="font-medium text-right max-w-[200px]">{address}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Description</span><span className="font-medium text-right max-w-[200px] truncate" title={description}>{description}</span></div>
              </div>
              <div className="mt-6 p-4 bg-amber-500/10 rounded-xl border border-amber-500/20">
                <p className="text-sm text-amber-800">En publiant cette mission, elle sera visible par tous les artisans qualifiés autour de vous.</p>
              </div>
            </div>
            <div className="flex gap-3">
              <Button variant="secondary" onClick={() => setStep(2)} className="flex-1">Modifier</Button>
              <Button onClick={onSubmit} isLoading={isLoading} className="flex-1">{isLoading ? 'Création...' : 'Publier la mission'}</Button>
            </div>
          </div>
        )}
      </motion.div>
