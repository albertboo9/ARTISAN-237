...
// Onboarding simplifié - guide textuel avec progression
export function OnboardingTour() {
  ...
  const handleNext = () => {
    ...
    else { completeOnboarding(); setIsVisible(false); }
  };
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[90vw] max-w-md glass rounded-2xl p-5 shadow-xl border border-border/50">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-medium text-muted-foreground">Étape {step + 1}/{steps.length}</span>
        <button onClick={() => { skipOnboarding(); setIsVisible(false); }} className="text-xs text-muted-foreground hover:text-foreground">Passer</button>
      </div>
