...
export default function MarketplacePage() {
  ...
  return (
    <div className="relative h-screen w-full" style={{ isolation: 'isolate' }}>
      ...
      <AnimatePresence>
        {isLoading && (
          <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[99999] flex items-center justify-center bg-background">
              ...
              {showFilters && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <div className="border-t border-border/50 mt-2 pt-3 pb-1 space-y-3">
                    <div><label className="text-xs font-medium text-muted-foreground">Rayon : {radius} km</label><input type="range" min={1} max={50} value={radius} onChange={e => setRadius(Number(e.target.value))} className="w-full h-1.5 bg-surface-container rounded-full appearance-none accent-primary" /></div>
      ...
      <div className="absolute bottom-4 left-4 right-4 z-[9999] mx-auto max-w-sm pointer-events-none">
        <div className="pointer-events-auto">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="glass rounded-2xl p-4 shadow-lg">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-sm font-medium text-foreground">{filtered.length} artisan{filtered.length > 1 ? 's' : ''}</p><p className="text-xs text-muted-foreground">Douala, Cameroun</p></div>
              </div>
            ...
            <div className="space-y-1 max-h-[100px] overflow-y-auto">
              {filtered.slice(0, 3).map((a: any, i: number) => (
                <button key={a.id || i} onClick={() => setSelected(a)} className="flex w-full items-center gap-3 p-1.5 rounded-xl hover:bg-surface-container transition-colors">
                  ...
                  <div className="flex-1 text-left"><span className="text-sm font-medium">{a.firstName} {a.lastName}</span><div className="flex items-center gap-1 text-xs text-muted-foreground"><Star className="h-3 w-3 text-amber-500 fill-amber-500" />{a.rating}{a.distance != null ? <span> · {a.distance.toFixed(1)} km</span> : null}</div></div>
                </button>
              ))}
            </div>
          </motion.div>
      ...
      <AnimatePresence>
        {selected && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[99999] flex items-end sm:items-center justify-center bg-black/20 backdrop-blur-sm" onClick={() => setSelected(null)}>
